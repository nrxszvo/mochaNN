...
def plot_dataset_dist(bins, npys, labels, png, Hs, Ls, strides):
    ...
    for i, (npy, name, H, L, stride) in enumerate(zip(npys, labels, Hs, Ls, strides)):
        d = np.load(npy, allow_pickle=True).item()
        hist = hist_local_minima(d["solutions"], H, L, stride, bins)
    ...
    ax.set_xticks(x + width / 2, xlabels, fontsize=8)
...
if __name__ == "__main__":
    ...
    plot_dataset_dist(
        args.bins, args.npy, labels, args.png, args.H, args.L, args.stride
    )
